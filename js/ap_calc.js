...
// mix and mega base stat changes.
$("#p1 .item").bind("keyup change", function() {
...
var megaDelta = {
    ...
    'Alakazite':{'at':0, 'df':20, 'sa':40, 'sd':10, 'sp':20, 'weight':0, 'ability': 'Trace', 'type': '', 'skip': ['Alakazam-Mega']},
    'Altarianite':{'at':40, 'df':20, 'sa':40, 'sd':0, 'sp':0, 'weight':0, 'ability': 'Pixilate', 'type': 'Fairy', 'skip': ['Altaria-Mega']},
    'Ampharosite':{'at':20, 'df':20, 'sa':40, 'sd':20, 'sp':-10, 'weight':0, 'ability': 'Mold Breaker', 'type': 'Dragon', 'skip': ['Ampharos-Mega']},
    ...
    'Kangaskhanite':{'at':30, 'df':20, 'sa':20, 'sd':20, 'sp':10, 'weight':20, 'ability': 'Parental Bond', 'type': '', 'skip': ['Kangaskhan-Mega']},
...
function autoUpdateStats(p, item) {
    ...
    pokemonName = fullSetName.substring(0, fullSetName.indexOf(" ("));
    setName = fullSetName.substring(fullSetName.indexOf("(") + 1, fullSetName.lastIndexOf(")"));
    console.log(pokemonName + " " + setName)
    var pokeObj = $(p);
    console.log(pokeObj)
    var abilityObj = pokeObj.find(".ability");
    var itemObj = pokeObj.find(".item");
    ...
    if (pokemon) {
        if (megaDelta[item] && megaDelta[item].skip.indexOf(pokemonName) < 0) { //change stats based on mega stone
            for (let i = 0; i < STATS.length; i++) {
                pokeObj.find("." + STATS[i] + " .base").val(pokemon.bs[STATS[i]] + megaDelta[item][STATS[i]]);
            }
            abilityObj.val(megaDelta[item]['ability']);
            pokeObj.find(".weight").val(pokemon.w + megaDelta[item]['weight']);
            if (megaDelta[item].type) pokeObj.find(".type2").val(megaDelta[item]['type']);
        } else {
            for (let i = 0; i < STATS.length; i++) {
                pokeObj.find("." + STATS[i] + " .base").val(pokemon.bs[STATS[i]]);
            }

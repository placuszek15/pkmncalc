...
function calculateSMSS(gen, attacker, defender, move, field) {
    ...
    else if (attacker.hasAbility('Analytic') && (turnOrder !== 'first' || field.defenderSide.isSwitching === 'out')) {
        bpMods.push(0x14cd);
        desc.attackerAbility = attacker.ability;
    }
    ...
    if (move.hasType(items_1.getBerryResistType(defender.item)) &&
        (typeEffectiveness > 1 || move.hasType('Normal')) && !attacker.hasAbility('Unnerve', 'As One (Glastrier)', 'As One (Spectrier)')) {
        finalMods.push(0x800);
        desc.defenderItem = defender.item;
    }
    var protect = false;
